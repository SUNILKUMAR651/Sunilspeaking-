...
bad_history = """val historyParts = newMessages.map { Content(listOf(Part(if(it.isUser) "User: ${it.text}" else "Teacher: ${it.text}"))) }"""
...
bad_clean = """val cleanResponse = aiResponse.replace(Regex("^(Teacher: )"), "")"""
